...
def test_ceiling_provider_under_threshold():
    ...
    res = provider.complete_structured(Tier.FAST, [Message(role="user", content="hello")], MockSchema)
...
def test_ceiling_provider_exceeds_calls():
    ...
    inner = MockLlmProviderBuilder().with_response({"name": "test1"}).with_response({"name": "test2"}).build()
    provider = CeilingLlmProvider(inner, max_calls=1)

    # First call should succeed
    provider.complete_structured(Tier.FAST, [Message(role="user", content="hello")], MockSchema)
