...
* a future contributor or external reader can read the artifact and understand the system.
...
* **Readability as a curated artefact** — the schema is a visible deliverable; design choices read as architectural maturity (or lack of it).
...
* Bad, because the schema artefact is a generated byproduct, not a curated contract — weaker statement of intent for cross-language consumption.
...
* Good, because external readers (TS, Go, future contributors) read a curated file.
...
* Bad, because the artefact shape is at odds with the project's stated identity — a graph tool should emit graph data.
...
* Good, because the artefact shape matches the project's stated identity — a graph tool should emit graph data.
