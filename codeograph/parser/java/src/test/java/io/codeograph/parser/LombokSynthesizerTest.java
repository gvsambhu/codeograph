 ...
 *
 * <p>
 * Each test verifies that the synthesised methods appear in the assembled
 * methods array with {@code is_synthesised: true} and the expected signature.
 *
 * <p>
 * All assertions are left as TODOs for the learner (Issue #2). Tests will
 * compile and run green (no-op synthesizer) until the learner implements
...
class LombokSynthesizerTest {
	...
	private static JSONObject findConstructor(JSONObject env) {
		...
		for (int i = 0; i < methods.length(); i++) {
			JSONObject m = methods.getJSONObject(i);
			if (m.getBoolean("is_constructor"))
				return m;
		}
	...
	private static long countConstructors(JSONObject env) {
		...
		long count = 0;
		for (int i = 0; i < methods.length(); i++) {
			if (methods.getJSONObject(i).getBoolean("is_constructor"))
				count++;
		}
		return count;
	}
