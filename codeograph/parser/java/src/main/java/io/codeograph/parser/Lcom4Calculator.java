 ...
 * </ul>
 *
 * <p>Operates entirely on finished JSON arrays — no AST access required (Option B).
 * Each method entry must carry {@code accessed_fields}, {@code calls},
 * {@code modifiers}, and {@code name} keys (populated by {@link ParsedFileAssembler}).
 *
...
final class Lcom4Calculator {
     ...
     *
     * @param methodsArr JSON array where each element is a method/constructor fact
    ...
    /**
     * Extract the simple callee name from a raw {@link com.github.javaparser.ast.expr.MethodCallExpr#toString()} string.
     *
     * <pre>
     *   "clean()"                → "clean"
     *   "this.save(user)"        → "save"
