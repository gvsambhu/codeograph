 ...
 *
 * <p>Dispatches to per-type builders (class, interface, enum, record, annotation),
 ...
 *
 * <p>The output envelope is the intermediate format consumed by the Python graph
...
final class ParsedFileAssembler {
    ...
    private static JSONObject buildClass(
        ...
        String stereotype = annotationList.stream()
                .filter(ParserConstants.STEREOTYPES::contains)
                .findFirst()
                .orElse(null);
    ...
    private static JSONObject buildEnum(
        ...
        // EnumDeclaration is not a ClassOrInterfaceDeclaration; buildFields/buildMethods
        // cannot accept it. Spring enums are typically constants-only; add overloads in v1.1.
     ...
     * (e.g. "private int x, y;"). Emits one JSON object per variable.
     *
     * Required output per field (maps to FieldNode in graph.schema.json):
     *   id, name, type, modifiers, annotations, is_autowired, is_id,
     *   injection_type, qualifier, generation, column, constraints
     */
    static JSONArray buildFields(ClassOrInterfaceDeclaration decl, String classFqcn) {
        ...
        for (FieldDeclaration fieldDecl : decl.getFields()) {
            ...
            boolean isAutowired = annotationNames.stream()
                    .anyMatch(ParserConstants.AUTOWIRE_ANNOTATIONS::contains);
            ...
            annotationNames.stream()
                    .filter(ParserConstants.CONSTRAINT_ANNOTATIONS::contains)
     ...
     *
     * Covers both regular {@link MethodDeclaration}s and {@link ConstructorDeclaration}s.
     * Each entry carries complexity metrics (via {@link ComplexityCalculator}) and
     * {@code accessed_fields} for LCOM4 (consumed by {@link Lcom4Calculator}).
     */
    static JSONArray buildMethods(ClassOrInterfaceDeclaration decl, String classFqcn) {
        ...
        for (MethodDeclaration method : decl.getMethods()) {
            ...
            for (NameExpr ne : method.findAll(NameExpr.class)) {
                ...
                if (declaredFieldNames.contains(name)) accessedFieldSet.add(name);
            }
            for (FieldAccessExpr fae : method.findAll(FieldAccessExpr.class)) {
                String name = fae.getNameAsString();
                if (declaredFieldNames.contains(name)) accessedFieldSet.add(name);
            }
            for (String name : accessedFieldSet) accessedFields.put(name);
     ...
     *   name, type, validate, constraints, binding
     */
