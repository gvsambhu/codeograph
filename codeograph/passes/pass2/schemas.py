...
class CrossDomainDependency(BaseModel):
    from_class: str = Field(
        description="The fully-qualified class name that holds the dependency."
    )
    from_domain: str = Field(
        description="The business domain the source class belongs to."
    )
    to_class: str = Field(
        description="The class being injected or imported from another domain."
    )
    to_domain: str = Field(
        description="The business domain the target class belongs to."
    )
...
class SynthesisResult(BaseModel):
    description: str = Field(
        description="2-3 sentences describing what the system does in plain business English."
    )
    ...
    domains: list[str] = Field(
        description="Echoed list of business domain names from the prompt."
    )
